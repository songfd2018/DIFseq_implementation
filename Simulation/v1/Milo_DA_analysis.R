...
#######################################
## Find groups
da_results <- groupNhoods(sim_milo, da_results, max.lfc.delta = 2)
head(da_results)
...
table(cluster_milo,metadata$celltype)
...
rownames(metadata) <- colnames(sim_milo)
...
write.table(DE_ROC_milo, file = paste0("Output/",method,"_cell_type_specific_DE_genes_for_ROC_",proj,"_v",ver,".txt"))