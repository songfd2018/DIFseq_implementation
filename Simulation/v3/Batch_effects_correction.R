...
table(paste0("CellType",metadata$celltype,"_Cond", metadata$treatment),
      simulation.combined@meta.data$seurat_clusters)
# table(metadata$treatment, simulation.combined@meta.data$seurat_clusters)
...
for(k in 0:Seurat_ntype){
  for(t in 2:3){
    ref_cells <- simulation.combined$condition == 1 & simulation.combined$seurat_clusters == k
    com_cells <- simulation.combined$condition == t & simulation.combined$seurat_clusters == k
    if(sum(ref_cells) > 10 & sum(com_cells) > 10){
      celltype.DE <- FindMarkers(simulation.combined,
                                logfc.threshold = 0,
                                ident.1 = paste0(k,"_Cond1"),
                                ident.2 = paste0(k,"_Cond",t), verbose = FALSE)

      order_genes <- order(factor(rownames(celltype.DE), levels = paste0("Genes-",1:G)))
      padj_Seurat <- cbind(padj_Seurat, celltype.DE$p_val_adj[order_genes])
      true_DE <- cbind(true_DE, E_syn[, celltype_matching[k+1] + (t-2) * K])

      type_condition_names <- c(type_condition_names,
                                paste0("Seurat_",k,"_True_",celltype_matching[k+1],"_Cond",t))
    }
  }
...
roc_DE_DIFseq <- roc(DE ~ pval, DE_ROC_Seurat, smooth = TRUE,
                     plot=TRUE, auc.polygon=TRUE, max.auc.polygon=TRUE, grid=FALSE,
                     auc.polygon.col="skyblue", print.auc=TRUE, xlim = c(1,0))

colnames(padj_Seurat) <- type_condition_names
colnames(true_DE) <- type_condition_names
...
rownames(PCA_output) <- rownames(metadata)
colnames(PCA_output) <- paste0("PC",1:50)

rownames(UMAP_output) <- rownames(metadata)
