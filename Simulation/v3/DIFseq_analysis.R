...
# color key
break_eta <- seq(-1.5,1.5, length.out = 101)
...
# color key
break_nu <- seq(-3.2,3.2, length.out = 101)
colorsChoice <- colorRampPalette(c("#1D78B2","#F2F2F2","#C4012D"))
batch_key <- colorsChoice(100)

jpeg(paste0("Images/heatmap_nu_syn_v",ver,".jpg"),
     width = 480, height = 960)
heat_nu <- heatmap_effects(nu.syn, 
                              color_by_batch,
                              break_effects = break_nu, 
                              color_key = batch_key)
dev.off()

jpeg(paste0("Images/colorkey_nu_syn_v",ver,".jpg"),width = 480, height = 240)
draw_keys(nu.syn,
          color_by_batch,
          break_effects = break_nu, 
          color_key = batch_key)
...
jpeg(paste0("Images/heatmap_nu_est_v",ver,".jpg"),
     width = 480, height = 960)
heat_logmu <- heatmap_effects(nu_est, 
                              color_by_batch,
                              break_effects = break_nu, 
                              color_key = batch_key)
dev.off()
...
pdf(paste0("Images/scatter_plot_delta_est_v",ver,".pdf"), width = 8, height = 8)
old_par <- par(mar = c(5,5,4,2) + 0.1)
plot(delta.syn,delta_est, type = "p",xlab = expression(paste("Ture ",delta)), ylab = expression(paste("Estimated ",delta)), cex = 1, cex.lab = 3, cex.axis= 2)
...
# color key
break_y <- seq(0,8.5, length.out = 101)
# colorsChoice <- colorRampPalette(c("#1D78B2","#F2F2F2","#C4012D"))
# treatment_key <- colorsChoice(100)
...
DA_Cond_ref1 <- Diff.Abundance(pi_ordered, w_MC, meta = metadata, 
                                ref = 1, dim = "Cond", subset = c(1,2,3))


# Condition 1 vs Condition 2
Stat_cond_1vs2 <- matrix(NA, 2, K_opt)
Stat_cond_1vs2[1,] <- c(DA_Cond_ref5[1,c(1,3,5,7)], DA_Cond_ref1[1,7])
Stat_cond_1vs2[2,] <- -Stat_cond_1vs2[1,]

pval_1vs2 <- matrix(NA, 2, K_opt)
log_pval <- -log10(c(DA_Cond_ref5[1,c(2,4,6,8)], DA_Cond_ref1[1,8]))
log_pval <- ifelse(log_pval > 100, 100, log_pval)
pval_1vs2[1,] <- log_pval * sign(Stat_cond_1vs2[1,])
pval_1vs2[2,] <- -pval_1vs2[1,]

metadata_df$DA_Stats_1vs2 <- 0
metadata_df$log10pval_1vs2 <- 0
for(t in 1:2){
  Cond_pair <- c(1,2)
  for(k in 1:K_opt){
    index_interest <- (metadata$celltype == k) & (metadata$condition == Cond_pair[t])
    metadata_df$DA_Stats_1vs2[index_interest] <- Stat_cond_1vs2[t,k]
    metadata_df$log10pval_1vs2[index_interest] <- pval_1vs2[t,k]
  }
}

# Condition 1 vs Condition 3
Stat_cond_1vs3 <- matrix(NA, 2, K_opt)
Stat_cond_1vs3[1,] <- c(DA_Cond_ref5[2,c(1,3,5,7)], DA_Cond_ref1[2,7])
Stat_cond_1vs3[2,] <- -Stat_cond_1vs3[1,]

pval_1vs3 <- matrix(NA, 2, K_opt)
log_pval <- -log10(c(DA_Cond_ref5[2,c(2,4,6,8)], DA_Cond_ref1[2,8]))
log_pval <- ifelse(log_pval > 100, 100, log_pval)
pval_1vs3[1,] <- log_pval * sign(Stat_cond_1vs3[1,])
pval_1vs3[2,] <- -pval_1vs3[1,]

metadata_df$DA_Stats_1vs3 <- 0
metadata_df$log10pval_1vs3 <- 0
for(t in 1:2){
  Cond_pair <- c(1,3)
  for(k in 1:K_opt){
    index_interest <- (metadata$celltype == k) & (metadata$condition == Cond_pair[t])
    metadata_df$DA_Stats_1vs3[index_interest] <- Stat_cond_1vs3[t,k]
    metadata_df$log10pval_1vs3[index_interest] <- pval_1vs3[t,k]
  }
}

# Condition 2 vs Condition 3
Stat_cond_2vs3 <- matrix(NA, 2, K_opt)
Stat_cond_2vs3[1,] <- c(DA_Cond_ref5[3,c(1,3,5,7)], DA_Cond_ref1[3,7])
Stat_cond_2vs3[2,] <- -Stat_cond_2vs3[1,]

pval_2vs3 <- matrix(NA, 2, K_opt)
log_pval <- -log10(c(DA_Cond_ref5[3,c(2,4,6,8)], DA_Cond_ref1[3,8]))
log_pval <- ifelse(log_pval > 100, 100, log_pval)
pval_2vs3[1,] <- log_pval * sign(Stat_cond_2vs3[1,])
pval_2vs3[2,] <- -pval_2vs3[1,]

metadata_df$DA_Stats_2vs3 <- 0
metadata_df$log10pval_2vs3 <- 0
for(t in 1:2){
  Cond_pair <- c(2,3)
  for(k in 1:K_opt){
    index_interest <- (metadata$celltype == k) & (metadata$condition == Cond_pair[t])
    metadata_df$DA_Stats_2vs3[index_interest] <- Stat_cond_2vs3[t,k]
    metadata_df$log10pval_2vs3[index_interest] <- pval_2vs3[t,k]
  }
}
...
feature1 <- "UMAP1"
feature2 <- "UMAP2"
color_by <- "log10pval_1vs2"
# color_group <- c("green","blue","red")
...
p <- ggplot(metadata_df, aes_string(x= feature1, y= feature2, color = color_by)) +
  ...
  theme(axis.text.x = element_text(# face = "bold", #color = "#993333", 
    ...
    legend.position = "top",
    legend.text = element_text(size = 24),
    legend.title = element_text(size = 28, face="bold", vjust = 1),
    panel.background = element_rect(colour = "black",size = 2)) +
